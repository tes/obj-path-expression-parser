...
function * iterCustomFragment (iterPath, fragment, obj, customFunctions) {
  const expression = fragment.fragment
  const funcName = expression.substr(0, expression.indexOf(' '))
  const funcArgument = expression.substr(expression.indexOf(' ') + 1)
  ...
  for (const path of iterPath) {
    ...
    yield * customFunc(path, funcArgument, parent)
  }
}
...
function * iterPathExpression (iterPath, pathExpression, obj, customFunctions) {
  ...
  for (const fragment of pathExpression.expression) {
    if (fragment._type === grammar.unescapedFragment.name || fragment._type === grammar.escapedFragment.name) {
      ...
      currentIterPath = iterCustomFragment(currentIterPath, fragment, obj, customFunctions)
    } else if (fragment._type === grammar.pathExpressions.name) {
      currentIterPath = iterPathExpressions(currentIterPath, fragment, obj, customFunctions)
    } else {
...
function * iterPathExpressions (iterPath, pathExpressions, obj, customFunctions) {
  const paths = Array.from(iterPath)
  const iterables = pathExpressions.expressions.map((pathExpression) =>
    iterPathExpression(paths, pathExpression, obj, customFunctions))
